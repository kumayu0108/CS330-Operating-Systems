...
{	
	...
	if(current_addr == (u64)dbg_info->end_handler){
		...
		*((u8*)(ctx->regs.entry_rip)) = INT3_OPCODE;
		...
		ctx->regs.entry_rsp = ctx->regs.entry_rsp - sizeof((u64)(toRem->retAddr));
		*((u64*)(ctx->regs.entry_rsp)) = toRem->retAddr;
		...
		if(MAX_BACKTRACE >= 1){
			...
			do{
				...
				if(END_ADDR != curr_addr){
					curr = fnHead;
					struct funcStack *efg = NULL;
					while(curr != NULL){if(curr->stackAddr == (head + 1)){efg = curr;} curr = curr->next;}
					if(efg != NULL)bt[i] = efg->retAddr;
					else bt[i] = (curr_addr == (u64)parent_ctx->dbg->end_handler ? *(head + 1) : curr_addr);
	...
	if(found){
		...
		*((u8*)(ctx->regs.entry_rip)) = INT3_OPCODE;
		...
		curr->retAddr = *((u64*)(ctx->regs.entry_rsp));
		curr->stackAddr = (u64*)(ctx->regs.entry_rsp);
		dbg_info->RSP = ctx->regs.entry_rsp;
